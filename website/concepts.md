...
This is the **why** behind `@hilaryosborne/sourcing`. Grasp it and the parts compose themselves; skip it and you will misuse them. The one-line thesis:
...
See [the events guide](/guide/events) for the how-to.
...
See [the aggregates guide](/guide/aggregates) for the how-to.
...
There is no separate `initial` seed. Handlers are typed `(current: State, event) => State` — a _complete_ `current`, not a `Partial`. You keep that promise by seeding the full model in your _creating_ event (e.g. `*.opened.v1`). Break it — make a projection whose first folded event doesn't establish the whole shape — and you get a runtime validation error the types said couldn't happen. That sharp edge is the deliberate price of not writing `current.x | undefined` in every handler. See [the projections guide](/guide/projections) for how to stay on the right side of it.
:::
...
See [the storage adapters guide](/guide/storage-adapters) for the how-to.
