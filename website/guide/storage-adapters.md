...
If you want the mental model behind events, aggregates, and projections before wiring storage, start with [Concepts](/concepts).
...
`forget` is idempotent and convergent, but it is **not atomic** across its steps. If it fails after overwrite but before binning projections, PII can persist in a cached "current" projection. **Re-run it until it succeeds.**
:::
...
Multi-event append is wrapped in a transaction, and Mongo has no single-statement multi-document atomic write. You need a **replica set** (a single-node one is enough for local/dev). Without it, the append transaction will fail.
:::

The port exposes ops + `transaction` + `ensureUniqueIndex`:
...
Wiring logging, error reporting, and profiling is covered in [Observability](/guide/observability).
...
See also [Aggregates](/guide/aggregates) and [Projections](/guide/projections).
