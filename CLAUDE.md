...
- **Versioning is a naming convention, not a feature.** Topics are opaque unique strings (`file.create.v1`). The library does not parse, relate, or understand versions. No upcasters, no version field, no migration machinery. If a consumer makes a breaking change without versioning their topic, that is their problem, not ours to catch.
